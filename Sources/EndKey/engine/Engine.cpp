...
// Full list of special characters that trigger macro expansion
// Includes all punctuation, symbols, and control keys
...
static int capsElem = 0;        // Fix: Initialize to 0
static int key = 0;             // Fix: Initialize to 0
static int markElem = 0;        // Fix: Initialize to 0
...
static bool _skipMacroNextBreak = false; //for Cmd/Alt cancel macro feature
...
void checkSpelling(const bool& forceCheckVowel=false) {
    ...
    if (_spellingEndIndex > 0) {
        ...
        if (CHR(VSI) == KEY_U && k > 0 && k < _spellingEndIndex-1 && CHR(VSI-1) == KEY_Q) {
            ...
            j = k;
            VSI = k;
        } else if (_index >= 3 && CHR(0) == KEY_G && CHR(1) == KEY_I && IS_CONSONANT(CHR(2))) {
        ...
        if (k > j) { //has vowel,
            ...
            if (k - j > 1 && forceCheckVowel && j < _index && _vowelCombine.find(CHR(j)) != _vowelCombine.end()) {
            ...
            } else if (j < _index && !IS_CONSONANT(CHR(j))) {
                _spellingVowelOK = true;
            }
            ...
            //limit: end consonant "ch", "t" can not use with "~" (MARK4), "`" (MARK2), "?" (MARK3)
            if (_spellingOK) {
                // Fix: Check if has invalid marks (MARK2, MARK3, MARK4) but NOT (MARK1 or MARK5)
                if (_index >= 3 && CHR(_index-1) == KEY_H && CHR(_index-2) == KEY_C) {
                    Uint32 markFlags = (_index >= 4) ? (TypingWord[_index-3] & MARK_MASK) : 0;
                    if (markFlags && !(markFlags & (MARK1_MASK | MARK5_MASK))) {
                        _spellingOK = false;
                    }
                } else if (_index >= 2 && CHR(_index-1) == KEY_T) {
                    Uint32 markFlags = (_index >= 3) ? (TypingWord[_index-2] & MARK_MASK) : 0;
                    if (markFlags && !(markFlags & (MARK1_MASK | MARK5_MASK))) {
                        _spellingOK = false;
                    }
                }
        ...
        for (i = _index-1; i >= 0; i--) {
            if (CHR(i) == KEY_N || CHR(i) == KEY_C || CHR(i) == KEY_I ||
                ...
                if (i - 2 >= 0 && CHR(i - 1) == KEY_O && CHR(i - 2) == KEY_U) {
                    if (i >= 2 && ((TypingWord[i-1] & TONEW_MASK) ^ (TypingWord[i-2] & TONEW_MASK))) {
...
void checkCorrectVowel(vector<vector<Uint16>>& charset, int& i, int& k, const Uint16& markKey) {
    ...
    // Fix: Check both k >= 0 AND k+1 < _index to prevent buffer overflow
    if (isCorect && k >= 0 && k + 1 < _index) {
        if (CHR(k) == CHR(k+1)) {
            isCorect = false;
        }
    }
...
bool canHasEndConsonant() {
    // Check if VSI is valid and key exists in _vowelCombine
    if (VSI >= _index || _vowelCombine.find(CHR(VSI)) == _vowelCombine.end()) {
        return false;
    }
    ...
    for (ii = 0; ii < vo.size(); ii++) {
        ...
        for (iii = 1; iii < vo[ii].size(); iii++) {
            // Fix: Check kk bounds before accessing TypingWord[kk]
            if (kk > VEI || kk >= _index || ((CHR(kk) | (TypingWord[kk] & TONE_MASK) | (TypingWord[kk] & TONEW_MASK)) != vo[ii][iii])) {
                break;
            }
            kk++;
        }
...
void handleModernMark() {
    ...
    if (vowelCount == 3 && VSI + 2 < _index &&
        ...
        VWSM = VSI;
        hBPC = _index - VWSM;
    } else if (VEI >= 1 && VEI < _index && CHR(VEI-1) == KEY_A && CHR(VEI) == KEY_Y) {
        VWSM = VEI - 1;
        hBPC = (_index - VEI) + 1;
    } else if (VSI + 1 < _index && CHR(VSI) == KEY_U && CHR(VSI+1) == KEY_O) {
        VWSM = VSI + 1;
        hBPC = _index - VWSM;
    } else if (VSI + 1 < _index && (CHR(VSI+1) == KEY_O || CHR(VSI+1) == KEY_U)) {
        VWSM = VEI - 1;
        hBPC = (_index - VEI) + 1;
...
void handleOldMark() {
    //default
    if (vowelCount == 0 && VEI < _index && CHR(VEI) == KEY_I)
        VWSM = VEI;
    else
        VWSM = VSI;
    hBPC = (_index - VWSM);
    ...
    //rule 3
    for (ii = VSI; ii <= VEI && ii < _index; ii++) {
...
void insertMark(const Uint32& markMask, const bool& canModifyFlag) {
    ...
    //send data
    // Fix: Prevent buffer underflow - ensure kk stays >= 0
    kk = _index - 1 - VSI;
    if (kk < 0 || VSI >= _index) {
        // Safety check: invalid vowel range
        return;
    }
...
void handleMainKey(const Uint16& data, const bool& isCaps) {
    ...
    //check Vowel
    if (vInputType == vVNI) {
        VEI = 0; // Fix: Initialize VEI before loop
        for (i = _index-1; i >= 0; i--) {
    ...
    // Fix: Check VEI bounds before accessing TypingWord[VEI]
    keyForAEO = ((vInputType != vVNI) ? data : ((data == KEY_7 || data == KEY_8 ? KEY_W : (data == KEY_6 && VEI < _index ? TypingWord[VEI] : data))));
...
void vSkipMacroNextBreak() {
    // Only activate if currently typing a word
    if (_index > 0) {
    ...
    if (_index > 0) {
        if (vQuickStartConsonant && _quickStartConsonant.find(CHR(0)) != _quickStartConsonant.end()) {
            ...
            if (_index < MAX_BUFF-1)
                _index++;
            //right shift
            for (i = _index-1; i >= 2 && i < MAX_BUFF; i--) {
                TypingWord[i] = TypingWord[i-1];
            }
            // Fix: Check bounds before accessing TypingWord[2]
            bool hasCaps2 = (_index >= 3 && (TypingWord[2] & CAPS_MASK));
            if (_index >= 2) {
                TypingWord[1] = _quickStartConsonant[CHR(0)][1] | ((TypingWord[0] & CAPS_MASK) && hasCaps2 ? CAPS_MASK : 0);
            }
...
void vEnglishMode(const vKeyEventState& state, const Uint16& data, const bool& isCaps, const bool& otherControlKey) {
    ...
    if (state == vKeyEventState::MouseDown || (otherControlKey && !isCaps)) {
        ...
        if (isMacroBreakCode(data)) {
            // Check macro before clearing on break codes
...
void vKeyHandleEvent(const vKeyEvent& event,
    ...
    if ((IS_NUMBER_KEY(data) && capsStatus == 1)
        ...
        //check macro feature
        if (vUseMacro && isMacroBreakCode(data) && !_hasHandledMacro && !_skipMacroNextBreak && findMacro(hMacroKey, hMacroData)) {
            hCode = vReplaceMaro;
            hBPC = (Byte)hMacroKey.size();
            _hasHandledMacro = true;
        } else if ((vQuickStartConsonant || vQuickEndConsonant) && !tempDisableKey && !_skipMacroNextBreak && isMacroBreakCode(data)) {
            checkQuickConsonant();
        ...
        // When user hits space twice consecutively after a word, replace second space with ". "
        // Only trigger if _index > 0 (has typed word) to avoid triggering after punctuation
        if (vDoubleSpacePeriod && _spaceCount == 1 && _index > 0) {
            ...
            vCheckSpelling = _useSpellCheckingBefore;
            _skipMacroNextBreak = false;
            return; // Early return, skip normal space processing
        }

        if (vUseMacro && !_hasHandledMacro && !_skipMacroNextBreak && findMacro(hMacroKey, hMacroData)) { //macro
            hCode = vReplaceMaro;
            hBPC = (Byte)hMacroKey.size();
            _spaceCount++;
            _hasHandledMacro = true;
        } else if ((vQuickStartConsonant || vQuickEndConsonant) && !tempDisableKey && !_skipMacroNextBreak && checkQuickConsonant()) {
            _spaceCount++;
        ...
        vCheckSpelling = _useSpellCheckingBefore;
        _skipMacroNextBreak = false; // Reset flag after space
    } else if (data == KEY_DELETE) {
        hCode = vDoNothing;
        hExt = 2; //delete
        if (_specialChar.size() > 0) {
        ...
        // Reset skip macro flag when typing additional characters
        if (_skipMacroNextBreak) {
            _skipMacroNextBreak = false;
        }
        ...
        if (vUseMacro) {
            if (hCode == vDoNothing) {
                ...
                // Rebuild hMacroKey from current TypingWord state
                // This fixes bug where transformed characters (e.g., 'ơ')
                // were incorrectly added to macro key tracking
                hMacroKey.clear();
                for (i = 0; i < _index; i++) {
        ...
        if (vUseMacro && isMacroBreakCode(data)) {
