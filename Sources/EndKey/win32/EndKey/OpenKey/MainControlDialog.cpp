...
void MainControlDialog::initDialog() {
    ...
    wsprintfW(titleBuffer, title, EndKeyHelper::getVersionString().c_str());
    SetWindowText(hDlg, titleBuffer);

    //input type
    comboBoxInputType = GetDlgItem(hDlg, IDC_COMBO_INPUT_TYPE);
    vector<LPCTSTR>& inputType = EndKeyManager::getInputType();
    ...
    vector<LPCTSTR>& tableCode = EndKeyManager::getTableCode();
    ...
    checkTempOffEndKey = GetDlgItem(hTabPage1, IDC_CHECK_TEMP_OFF_OPEN_KEY);
    createToolTip(checkTempOffEndKey, IDS_STRING_TEMP_OFF_OPENKEY);
...
INT_PTR MainControlDialog::eventProc(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_COMMAND: {
        ...
        case IDBUTTON_OK:
            ...
            break;
        case IDBUTTON_EXIT:
            AppDelegate::getInstance()->onEndKeyExit();
            break;
        case ID_BTN_DEFAULT: {
        ...
        case IDC_BUTTON_GO_SOURCE_CODE:
            ShellExecute(NULL, _T("open"), _T("https://github.com/tuyenvm/EndKey"), NULL, NULL, SW_SHOWNORMAL);
            break;
        default:
            if (HIWORD(wParam) == CBN_SELCHANGE) {
                ...
                _lastKeyCode = EndKeyManager::_lastKeyCode;
                if (_lastKeyCode > 0) {
                    EndKeyManager::_lastKeyCode = 0;
    ...
    case WM_NOTIFY: {
        ...
        case NM_RETURN: {
            ...
            if (link->hdr.idFrom == IDC_SYSLINK_HOME_PAGE)
                ...
                ShellExecute(NULL, _T("open"), _T("https://www.facebook.com/EndKeyVN"), NULL, NULL, SW_SHOWNORMAL);
...
void MainControlDialog::fillData() {
    ...
    SendMessage(checkTempOffEndKey, BM_SETCHECK, vTempOffEndKey ? 1 : 0, 0);
    ...
    //tab info
    wchar_t buffer[256];
    wsprintfW(buffer, _T("Phiên bản %s cho Windows - Ngày cập nhật: %s"), EndKeyHelper::getVersionString().c_str(), _T(__DATE__));
...
void MainControlDialog::onComboBoxSelected(const HWND& hCombobox, const int& comboboxId) {
    if (hCombobox == comboBoxInputType) {
        ...
        if (vRememberCode) {
            setAppInputMethodStatus(EndKeyHelper::getFrontMostAppExecuteName(), vLanguage | (vCodeTable << 1));
...
void MainControlDialog::onCheckboxClicked(const HWND& hWnd) {
    ...
    if (hWnd == checkCtrl) {
        ...
        EndKeyHelper::registerRunOnStartup(vRunWithWindows);
    }
        ...
        if (val)
            EndKeyManager::createDesktopShortcut();
    }
    ...
    else if (hWnd == checkTempOffEndKey) {
        val = (int)SendMessage(hWnd, BM_GETCHECK, 0, 0);
        APP_SET_DATA(vTempOffEndKey, val ? 1 : 0);
    }
...
void MainControlDialog::onUpdateButton() {
    ...
    string newVersion;
    if (EndKeyManager::checkUpdate(newVersion)) {
        WCHAR msg[256];
        wsprintf(msg,
            TEXT("EndKey Có phiên bản mới (%s), bạn có muốn cập nhật không?"),
        ...
        if (msgboxID == IDYES) {
            ...
            wsprintf(path, TEXT("%s\\EndKeyUpdate.exe"), path);
        ...
        MessageBox(hDlg, _T("Bạn đang dùng phiên bản mới nhất!"), _T("EndKey Update"), MB_OK);
    }
    EnableWindow(hUpdateButton, true);
}

void MainControlDialog::requestRestartAsAdmin() {
    EndKeyHelper::registerRunOnStartup(false);
    if (vRunAsAdmin && !IsUserAnAdmin()) {
        int msgboxID = MessageBox(
            hDlg,
            _T("Bạn cần phải khởi động lại EndKey để kích hoạt chế độ Admin!\nBạn có muốn khởi động lại EndKey không?"),
            _T("EndKey"),
            MB_ICONEXCLAMATION | MB_YESNO
        );
        if (msgboxID == IDYES) {
            PostQuitMessage(0);
            ShellExecute(0, L"runas", EndKeyHelper::getFullPath().c_str(), 0, 0, SW_SHOWNORMAL);
        }
    }
    else {
        EndKeyHelper::registerRunOnStartup(vRunWithWindows);
