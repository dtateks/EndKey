...
int AppDelegate::run(HINSTANCE hInstance) {
	...
	while (GetMessage(&msg, nullptr, 0, 0))	{
		if (msg.message == WM_KEYDOWN) {
			EndKeyManager::_lastKeyCode = (UINT16)msg.wParam;
		}
		if (!isDialogMsg(msg)) {
...
void AppDelegate::onToggleVietnamese() {
	...
	if (vUseSmartSwitchKey) {
		string& exe = EndKeyHelper::getLastAppExecuteName();
...
void AppDelegate::onEndKeyAbout() {
	if (aboutDialog == NULL) {
...
void AppDelegate::onEndKeyExit() {
	EndKeyManager::freeEngine();
