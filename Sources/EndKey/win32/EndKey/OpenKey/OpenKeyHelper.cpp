...
static LPCTSTR sk = TEXT("SOFTWARE\\TuyenMai\\EndKey");
static HKEY hKey;
...
static HANDLE _proc;
static string _exeNameUtf8 = "TheEndKeyProject";
...
int CF_OPENKEY = RegisterClipboardFormat(_T("EndKey Format"));

void EndKeyHelper::openKey() {
	LONG nError = RegEndKeyEx(HKEY_CURRENT_USER, sk, NULL, KEY_ALL_ACCESS, &hKey);
...
void EndKeyHelper::setRegInt(LPCTSTR key, const int & val) {
	openKey();
	RegSetValueEx(hKey, key, 0, REG_DWORD, (LPBYTE)&val, sizeof(val));
	RegCloseKey(hKey);
}

int EndKeyHelper::getRegInt(LPCTSTR key, const int & defaultValue) {
	openKey();
...
void EndKeyHelper::setRegBinary(LPCTSTR key, const BYTE * pData, const int & size) {
	openKey();
	RegSetValueEx(hKey, key, 0, REG_BINARY, pData, size);
	RegCloseKey(hKey);
}

BYTE * EndKeyHelper::getRegBinary(LPCTSTR key, DWORD& outSize) {
	openKey();
	if (_regData) {
		delete[] _regData;
		_regData = NULL;
...
void EndKeyHelper::registerRunOnStartup(const int& val) {
	if (val) {
		if (vRunAsAdmin) {
			string path = wideStringToUtf8(getFullPath());
			char buff[MAX_PATH];
			sprintf_s(buff, "schtasks /create /sc onlogon /tn EndKey /rl highest /tr \"%s\" /f", path.c_str());
			WinExec(buff, SW_HIDE);
		} else {
			RegEndKeyEx(HKEY_CURRENT_USER, _runOnStartupKeyPath, NULL, KEY_ALL_ACCESS, &hKey);
			wstring path = getFullPath();
			RegSetValueEx(hKey, _T("EndKey"), 0, REG_SZ, (byte*)path.c_str(), ((DWORD)path.size() + 1) * sizeof(TCHAR));
			RegCloseKey(hKey);
		}
	} else {
		RegEndKeyEx(HKEY_CURRENT_USER, _runOnStartupKeyPath, NULL, KEY_ALL_ACCESS, &hKey);
		RegDeleteValue(hKey, _T("EndKey"));
		RegCloseKey(hKey);
		WinExec("schtasks /delete  /tn EndKey /f", SW_HIDE);
	}
}

LPTSTR EndKeyHelper::getExecutePath() {
	if (!_hasGetPath) {
...
string& EndKeyHelper::getFrontMostAppExecuteName() {
	...
	if (wcscmp(_exeName, _T("EndKey64.exe")) == 0 ||
		wcscmp(_exeName, _T("EndKey32.exe")) == 0 || 
...
string & EndKeyHelper::getLastAppExecuteName() {
	if (!vUseSmartSwitchKey)
		return getFrontMostAppExecuteName();
	return _exeNameUtf8;
}

wstring EndKeyHelper::getFullPath() {
...
wstring EndKeyHelper::getClipboardText(const int& type) {
...
void EndKeyHelper::setClipboardText(LPCTSTR data, const int & len, const int& type) {
...
bool EndKeyHelper::quickConvert() {
...
DWORD EndKeyHelper::getVersionNumber() {
...
wstring EndKeyHelper::getVersionString() {
...
wstring EndKeyHelper::getContentOfUrl(LPCTSTR url){
	WCHAR path[MAX_PATH];
	GetTempPath2(MAX_PATH, path);
	wsprintf(path, TEXT("%s\\_EndKey.tempf"), path);
