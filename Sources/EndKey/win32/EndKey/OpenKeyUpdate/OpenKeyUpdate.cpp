...
DWORD WINAPI UpdateThreadFunction(LPVOID lpParam) {
	...
	wsprintf(path, TEXT("%s\\_EndKey.tempf"), currentDir);
	HRESULT res = URLDownloadToFile(NULL, L"https://raw.githubusercontent.com/tuyenvm/EndKey/master/version.json", path, 0, NULL);
	...
	if (res == S_OK) {
		...
		MessageBox(hDlg, _T("Có lỗi trong quá trình cập nhật, vui lòng thử lại sau!"), _T("EndKey Update"), MB_OK);
		ExitProcess(0);
		return 0;
	}
	...
	wsprintf(updateUrl, TEXT("https://github.com/tuyenvm/EndKey/releases/download/%s/EndKey%s-Windows.zip"),
		versionName.c_str(),
		versionName.c_str());
	wsprintf(path, TEXT("%s\\_EndKeyUpdate.zip"), currentDir);
	res = URLDownloadToFile(NULL, updateUrl, path, 0, NULL);

	if (res == S_OK) {
		//remove old file
		DeleteFile(L"EndKey64.exe");
		//extract zip file
		WinExec("powershell.exe -NoP -NonI -Command \"Expand-Archive '.\\_EndKeyUpdate.zip' '.\\_EndKeyUpdate'\" ", SW_HIDE);
		Sleep(5000);
		MoveFile(L"_EndKeyUpdate\\EndKey64.exe", L"EndKey64.exe");
		DeleteFile(path);
		DeleteFile(L"_EndKeyUpdate\\EndKeyUpdate.exe");
		DeleteFile(L"_EndKeyUpdate\\EndKey64.exe");
		DeleteFile(L"_EndKeyUpdate\\EndKey32.exe");
		RemoveDirectory(L".\\_EndKeyUpdate");
		MessageBox(hDlg, _T("Bạn đã cập nhật EndKey bản mới nhất thành công!"), _T("EndKey Update"), MB_OK);
		ExitProcess(0);
	} else {
		MessageBox(hDlg, _T("Có lỗi trong quá trình cập nhật, vui lòng thử lại sau!"), _T("EndKey Update"), MB_OK);
		ExitProcess(0);
