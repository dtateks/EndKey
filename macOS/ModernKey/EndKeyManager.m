...
extern void EndKeyInit(void);

extern CGEventRef EndKeyCallback(CGEventTapProxy proxy,
                                 CGEventType type,
                                 CGEventRef event,
                                 void *refcon);
...
@interface EndKeyManager ()

@end

@implementation EndKeyManager {
...
+(BOOL)initEventTap {
    ...
    if (!eventTap) {
        ...
        printf("ERROR: CGEventTapCreate failed. This usually means accessibility permissions are not granted.\n");
        return NO;
    } else {
        printf("SUCCESS: Event tap created successfully\n");
    }
    ...
    // Set it all running on background thread with dedicated run loop
...
+(void)checkNewVersion:(NSWindow*)parent callbackFunc:(CheckNewVersionCallback) callback {
    ...
    [[aSession dataTaskWithURL:[NSURL URLWithString:@"https://raw.githubusercontent.com/tuyenvm/EndKey/master/version.json"] completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
...
+(void)showUpdateMessage:(NSWindow*)parent needUpdating:(BOOL)needUpdating newVersion:(NSString*)versionString {
    NSAlert *alert = [[NSAlert alloc] init];
    [alert setMessageText:(needUpdating ? [NSString stringWithFormat:@"EndKey Có phiên bản mới (%@), bạn có muốn cập nhật không?", versionString] : @"Bạn đang dùng phiên bản mới nhất!")];
    [alert setInformativeText:(needUpdating ? @"Bấm 'Có' để cập nhật EndKey." : @"")];
...
+(void)launchUpdateHelper {
    ...
    NSString* target = [NSString stringWithFormat:@"%@/EndKeyUpdate.app", [self getApplicationSupportFolder]];
...
+(NSString*)getApplicationSupportFolder {
    ...
    return [NSString stringWithFormat:@"%@/EndKey", applicationSupportDirectory];
}

+(NSString*)getUpdateBundlePath {
    NSString *currentpath = [[NSBundle mainBundle] bundlePath];
    return [NSString stringWithFormat:@"%@/Contents/Library/LoginItems/EndKeyUpdate.app", currentpath];
}
@end
