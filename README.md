...
# [EndKey](http://open-key.org)
### [Download bản mới nhất](https://github.com/tuyenvm/EndKey/releases)
...
### Mã nguồn của ứng dụng được mở công khai, minh bạch dưới giấy phép GPL. Điều này nghĩa là bạn hoàn toàn có thể tải mã nguồn về tự build, cải tiến theo mục đích của bạn. Nếu bạn tái phân phối bản cải tiến của bạn, thì nó cũng phải là mã nguồn mở và thông báo bản gốc là EndKey.

### Lưu ý, khi sử dụng EndKey, bạn nên tắt hẳn bộ gõ khác vì 2 chương trình bộ gõ sẽ xung đột nhau, dẫn đến thao tác không chính xác.
...
- **Macro** - Tính năng gõ tắt vô cùng tiện lợi. Gõ tắt của macOS chỉ hỗ trợ 20 ký tự, còn EndKey không giới hạn ký tự.
- **Chuyển chế độ thông minh:** (On/Off) (Bản 1.2 về sau) - Bạn đang dùng chế độ gõ Tiếng Việt trên ứng dụng A, bạn chuyển qua ứng dụng B trước đó bạn dùng chế độ gõ Tiếng Anh, EndKey sẽ tự động chuyển qua chế độ gõ Tiếng Anh cho bạn, khi bạn quay lại ứng dụng A, EndKey tất nhiên sẽ chuyển lại chế độ gõ tiếng Việt, rất cơ động.
...
- **Cập nhật tự động:** (Bản 1.3 về sau) tính năng hỗ trợ cập nhật phiên bản EndKey mới nhất mỗi khi mở EndKey hoặc tự check trong phần mục Giới thiệu.
...
- **Tự ghi nhớ bảng mã theo ứng dụng:** (Bản 2.0.1 về sau) Phù hợp cho các bạn dùng Photoshop, CAD,... với các bảng mã VNI, TCVN3. EndKey tự ghi nhớ ứng dụng nào dùng bảng mã nào để lần sau sử dụng Photoshop, CAD,... EndKey có thể tự chuyển sang bảng mã đó.
- ...


[Changelog](https://github.com/tuyenvm/EndKey/blob/master/CHANGELOG.md)

## Cài đặt:
**Cài đặt thủ công:**  
Tải bản EndKey mới nhất từ [đây](https://github.com/tuyenvm/EndKey/releases/latest), mở file `dmg` ra rồi kéo thả `EndKey.app` vào thư mục `Application`.
...
Kiểm tra phiên bản EndKey:
```
$ brew info --cask openkey
```
Gõ lệnh sau để homebrew tự cài EndKey cho bạn:
```
$ brew install --cask openkey
```

Để update phiên bản mới nhất của EndKey
```
$ brew upgrade --cask openkey
```

## Note - Lưu ý:
EndKey cần cấp quyền, vào *System Preferences -> Security & Privacy -> Accessibility*, kích hoạt `EndKey.app`. **Không tắt nó khi đang dùng EndKey**.
...
- Fanpage: [https://www.facebook.com/EndKeyVN](https://www.facebook.com/EndKeyVN)

## Liên kết
- [EndKey cho Windows, xem chi tiết tại đây](https://github.com/tuyenvm/EndKey/tree/master/Sources/EndKey/win32)
- [EndKey cho Linux (đang phát triển)](https://github.com/tuyenvm/EndKey/tree/master/Sources/EndKey/linux)
## Một điều nhỏ nhoi
